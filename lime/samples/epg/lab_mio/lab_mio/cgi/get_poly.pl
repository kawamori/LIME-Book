...
# Use seconds part as minute part.
# my( $hhmm) = sprintf( "\%02d\%02d", (localtime())[2,0]);

# Static hour.
my( $hhmm) = sprintf( "\%02d\%02d", 17, (localtime())[0]);

# my( $hhmm) = "1750";

$hhmm = substr( $hhmm, 0, 3) . "[0-9]";

# SingHealth
map( { $pbPolySet{ $_} = 1; } qw( BDP BMP GLP MPP OUP PRP QTP SKP TMP));

# National Healthcare Group
map( { $pbPolySet{ $_} = 1; } qw( AMK BBK CCK HOU JUR TPY WDS YIS));

my( %pbDataType) = (TEXT => "txt", IMAGE => "jpg");

my( @pszQueueType) = qw( REGISTRATION CONSULTATION PHARMACY);
my( @pszQueueTypeNHG) = qw( reg cons pharm);
...
map( { $pbQueueArea{ $_} = 1; } @pszQueueTypeNHG);

my( $szQueueType) = "(" . join( "|", @pszQueueType) . ")";
my( $rxQueueInfo) = qr/^$szQueueType .* : (\d+) *(mins)?$/;

my( $szQueueTypeNHG) = "(" . join( "|", @pszQueueTypeNHG) . ")";
my( $rxQueueInfoNHG) =
    qr/\bqueueviewer_[a-z]{3}_$szQueueTypeNHG
       \_video\@\-\d{8}.*\-$hhmm\d{2}\.jpg\b/x;

########################################
sub main {
  (-e "$szPolyDataBaseDir") or
    die( "Error: path \"", $szPolyDataBaseDir, "\" does not exist.", $/);

  (-d "$szPolyDataBaseDir") or
    die( "Error: path \"", $szPolyDataBaseDir, "\" is not a directory.", $/);
  ...
  ####################
  my( $szGroup) = $q->url_param( 'group');
  defined( $szGroup) or ($szGroup = $q->param( 'group'));
  defined( $szGroup) or ($szGroup = 'singhealth');
  $szGroup = lc( $szGroup);

  my( $bCanned) = ($szGroup eq "nhg");

  my( $rxFilter) = $rxQueueInfo;

  if( $bCanned) {
    $rxFilter = $rxQueueInfoNHG;

    if( $gbDebug) {
      print( "looking for -", $hhmm, "??.jpg", $/);
    };
  };
  ...
  if( defined( $szQueueArea) && exists( $pbQueueArea{ $szQueueArea})) {
    ...
    if( $bCanned) {
      $szQueueArea = $pszRemapArea{ $szQueueArea};
    };
  ...
  if( defined( $szDataType) && ($szDataType eq "IMAGE")) {
    ...
    @pszRawData = map( { (split( /\t/, $_))[-1]; }
                       grep( /$szPattern/, @pszRawData));

    my( $szImageFile) = $pszRawData[ -1];

    if( $bCanned) {
      $szImageFile = (grep( /$rxQueueInfoNHG/, @pszRawData))[0];
    };

    if( !( $szImageFile)) {
      $szImageFile = $pszRawData[ -1];
    };
